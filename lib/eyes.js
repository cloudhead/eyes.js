...
var eyes = {
    ...
    inspect: function (obj, label, options) {
        ...
        return this.print(this.stringify(obj, options.styles), label, options);
    },
        ...
        switch (typeOf(obj)) {
            case "string":
                obj = (obj.length === 1 ? "'" + obj + "'" : '"' + obj + '"')
                      .replace(/\n/g, '\\n');
                return stylize(obj, styles.string);
            case "regexp"   : return stylize('/' + obj.source + '/', styles.regexp);
            case "number"   : return stylize(obj + '', styles.number);
